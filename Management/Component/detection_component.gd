...
signal player_detected(location,direction)
...
func _physics_process(_delta: float) -> void:
	if forward.is_colliding() or forward_up.is_colliding() or forward_down.is_colliding():
		...
		if forward_collider is Player:
			...
			player_detected.emit(location,direction)
			print("See ",forward.get_collider().get_class()," In location: ", location," Direction: ",direction)
